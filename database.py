...
class Database:
    ...
    def get_all_leaderboard_channels(self) -> List[Tuple[int, int]]:
        """Get all leaderboard channels for all guilds."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT guild_id, leaderboard_channel_id 
                FROM guild_config 
    ...
    def award_score(self, guild_id: int, user_id: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Mark claim as score awarded - get the most recent claim first
            cursor.execute('''
    ...
    def remove_score(self, guild_id: int, user_id: int):
        """Remove a claim score from a user."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                UPDATE leaderboard 
    ...
    def get_leaderboard(self, guild_id: int, period: str = "total") -> List[Tuple[int, int]]:
        """Get leaderboard for a guild by period (daily, weekly, total)."""
        column_map = {
            "daily": "daily_claims",
            "weekly": "weekly_claims", 
            "total": "total_claims"
        }
        
        column = column_map.get(period, "total_claims")
        
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(f'''
                SELECT user_id, {column} FROM leaderboard 
    ...
    def reset_daily_leaderboard(self):
        """Reset daily leaderboard for all guilds."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            today = datetime.now().date()
            cursor.execute('''
                UPDATE leaderboard 
                SET daily_claims = 0, last_daily_reset = ?
            ''', (today,))
            conn.commit()
    
    def reset_weekly_leaderboard(self):
        """Reset weekly leaderboard for all guilds."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            today = datetime.now().date()
            cursor.execute('''
                UPDATE leaderboard 
                SET weekly_claims = 0, last_weekly_reset = ?
                WHERE (julianday(?) - julianday(last_weekly_reset)) >= 7
            ''', (today, today))
            conn.commit()
    
    def set_ticket_holder(self, channel_id: int, user_id: int, set_by: int):
        """Set the ticket holder for a channel."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def get_ticket_holder(self, channel_id: int) -> Optional[int]:
        """Get the ticket holder ID for a channel."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def remove_ticket_holder(self, channel_id: int):
        """Remove the ticket holder for a channel."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def update_last_message(self, channel_id: int, user_id: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Get timeout info to determine if user is staff or holder
            cursor.execute('''
                SELECT claimer_id, ticket_holder_id FROM active_timeouts WHERE channel_id = ?
            ''', (channel_id,))
            result = cursor.fetchone()
            
            if result:
                claimer_id, ticket_holder_id = result
    ...
    def remove_timeout(self, channel_id: int):
        """Remove timeout tracking for a channel."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('DELETE FROM active_timeouts WHERE channel_id = ?', (channel_id,))
            conn.commit()
    
    def get_all_active_timeouts(self) -> List[tuple]:
        """Get all active timeout information."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT channel_id, claimer_id, ticket_holder_id, claim_time, 
                       last_staff_message, last_holder_message, original_permissions, officer_used
                FROM active_timeouts
            ''')
            return cursor.fetchall()
    
    def mark_officer_used(self, channel_id: int):
        """Mark that the officer command was used for this ticket."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
